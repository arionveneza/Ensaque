...
import type { ParadaDetalhe, TempoOrdem } from '@/dados/api-gestao'
import { formataHms } from '@/dominio/calculos'
import {
  Botao, Cartao, Erro, Pagina, Tabela, Tag, Vazio, exportarCsv, n, somaDias,
} from '@/componentes/ui'
...
export default function Indicadores() {
  ...
  useEffect(() => {
    let vivo = true
    setCarregando(true)
    Promise.all([g.listarTempos(de, hoje), g.paretoParadas(de, hoje)])
      .then(([t, p]) => {
        if (!vivo) return
        setTempos(t)
        setParadas(p)
