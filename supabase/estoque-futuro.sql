-- ============================================================
-- "Estoque futuro" (22/09/2026, pedido do Arion): estoque do SAP + o que
-- está planejado a produzir, mas SÓ contando ordem em status
-- `Aguardando lote`, `Pronto para produzir` ou `Qualidade apontada` — ele
-- foi explícito ("apenas"): nada de `Não programada`/`Programada` (ainda
-- não é compromisso — o PCP pode reprogramar/desprogramar sem custo) nem
-- `Em produção`/`Parada`/`Finalizada` (produção em curso, número ainda
-- pode mudar até a conferência de qualidade). `Qualidade apontada` entra
-- porque nesse ponto já produziu e já passou na qualidade — só falta o
-- lançamento no AGROTIS pra virar estoque de verdade no próximo upload.
--
-- `Aguardando lote`/`Pronto para produzir` são status DERIVADOS (nunca
-- ficam na coluna `ordens.status` — só existem em `v_ordens.status_efetivo`,
-- calculados de `maquina_id`/`confirmada_em`/`lote_liberado_em`). Em vez de
-- juntar com `v_ordens` (que tem um bug antigo e já conhecido: nunca emite
-- 'Excluida' em `status_efetivo`, achado na feature "Ordens sem caminhão"
-- de 19/09 — os chamadores se defendem olhando o status CRU também), replico
-- a mesma lógica direto em cima de `ordens` cru, com a MESMA exclusão de
-- Excluida que `abe` já tinha — nunca herda o bug por não passar por lá.
-- `maquina_id is not null and confirmada_em is not null`, sem checar
-- `lote_liberado_em`, cobre as DUAS (Aguardando lote OU Pronto pra
-- produzir) de propósito — ele quer as duas juntas.
--
-- Execute no SQL Editor do Supabase (idempotente). Base: a versão MAIS
-- RECENTE de v_balanco_demanda (pedido-faturado.sql, 21/09/2026) — grep -rl
...
abe as (select o.cultivar, r.nome as tratamento, o.embalagem,
               ...
               -- Aguardando lote / Pronto para produzir (maquina_id + confirmada_em
               -- setados, ainda não em produção) OU Qualidade apontada — nada de
               -- Nao programada/Programada/Em producao/Parada/Finalizada
               sum(o.bags) filter (where
                 o.status = 'Qualidade apontada'
                 or (o.status not in ('Em producao','Parada','Finalizada')
                     and o.maquina_id is not null and o.confirmada_em is not null)
               ) as planejado
