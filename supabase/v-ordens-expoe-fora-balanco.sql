-- ============================================================
-- v_ordens não mostrava fora_balanco (achado 25/08/2026)
-- Execute no SQL Editor do Supabase (idempotente)
-- ============================================================
--
-- Bug: `embalagem-peso-fixo.sql` recriou `v_ordens` com `select o.*, ...`
-- ANTES de `fora_balanco` existir em `ordens` (essa coluna só nasceu em
-- `ordem-fora-do-estoque.sql`, migração seguinte). `select o.*` numa view
-- é travado nas colunas que a tabela tinha no momento do CREATE — não
-- acompanha coluna nova adicionada depois. Resultado: a coluna está na
-- tabela e a view de balanço (`v_balanco_demanda`) já filtra por ela
-- corretamente, mas a TELA DE ORDENS nunca via o valor — a tag roxa
-- "sem estoque" nunca aparecia e o botão "sem estoque"/"volta ao
-- estoque" no menu de ações nunca trocava de nome, porque os dois lêem
-- `v_ordens`. O clique em "sem estoque" gravava certo no banco (dá pra
-- conferir direto na tabela `ordens`); só a leitura de volta é que não
-- mostrava.
--
-- Fix: recria a MESMA cadeia de views de `embalagem-peso-fixo.sql`,
-- sem mudar nada além de deixar o `select o.*` pegar a coluna que já
-- existe agora. DROP + CREATE (não CREATE OR REPLACE) pela mesma razão
-- de lá: replace não muda a lista de colunas herdada de `o.*` (42P16).
-- ============================================================
...
-- Conferência
-- ------------------------------------------------------------
select 'v_ordens agora expõe fora_balanco' as item,
       (position('fora_balanco' in pg_get_viewdef('tsi.v_ordens'::regclass)) > 0)::text as ok
union all
select 'DM1 aparece marcada fora do estoque (true, se você já clicou "sem estoque" nela)',
       (select fora_balanco::text from v_ordens where numero = 'DM1' limit 1)
union all
select 'view manteve security_invoker',
       (position('security_invoker=true' in array_to_string(c.reloptions, ',')) > 0)::text
  from pg_class c join pg_namespace ns on ns.oid = c.relnamespace
 where ns.nspname = 'tsi' and c.relname = 'v_ordens';
